...
class CustomQueryDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        self.dataset = data_args.custom_query_df
        self.preprocessor = QueryPreProcessor
        self.tokenizer = tokenizer
        self.q_max_len = data_args.q_max_len
        self.proc_num = data_args.dataset_proc_num
...
class CustomCorpusDataset:
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, cache_dir: str):
        self.dataset = data_args.custom_corpus_df
        self.preprocessor = CorpusPreProcessor
        self.tokenizer = tokenizer
        self.p_max_len = data_args.p_max_len
        self.proc_num = data_args.dataset_proc_num
    ...
    def process(self, shard_num=1, shard_idx=0):
        ...
        if self.preprocessor is not None:
            self.dataset = self.dataset.map(
                self.preprocessor(self.tokenizer, self.p_max_len, self.separator, "content"),
                batched=False,
                num_proc=self.proc_num,
                remove_columns=self.dataset.column_names,
                desc="Running tokenization",
            )
            # .remove_columns(["docid", "title"])
        return self.dataset


class CorpusPreProcessor:
    def __init__(self, tokenizer, text_max_length=256, separator=' ', field='text'):
        self.tokenizer = tokenizer
        self.text_max_length = text_max_length
        self.separator = separator
        if field == 'content':
            self.text_converter = lambda x: x["content"]
        else:
            self.text_converter = lambda x: x["title"] + self.separator + x["text"] if 'title' in x else x["text"]

    def __call__(self, example):
        docid = example['docid']
        text = self.text_converter(example)
